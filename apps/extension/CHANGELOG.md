...
- 📖 AI 용어 설명 톤 조정 — 완전 초보도 한 번에 이해되도록 전문용어 회피 · 일상 비유 유도. "직접 해보세요" 식 실습 유도 제거.

### Fixed
- 📦 패키지 설명 캐시에 "잘 모르겠어" 형태의 무답변이 저장되던 문제 수정. 활성화 시 과거 무답변 캐시 자동 정리.
- 🔎 `const`/`let` 등 예약어가 사용처 찾기 심볼로 선택되던 문제 수정.
- 💾 인라인 스레드의 `💾 사전에 저장` 링크와 🗑️ 휴지통 아이콘이 일부 스레드에서 사라지던 회귀 수정 (`isTrusted` 화이트리스트 → 전체 허용으로 복원, 모든 InlineThread에 `contextValue` 지정).
