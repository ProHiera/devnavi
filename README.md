...
| 📦 | **패키지 설명** | 워크스페이스 의존성 QuickPick → "뭐하는 거 · 언제 써 · 대안". 선택 영역 **아래 인라인**으로 표시. **두 번째부턴 캐시 (토큰 0원)** · "모름" 응답은 캐시 안 함. | `package.json` / `requirements.txt` / `pom.xml`에서 패키지명 드래그 → 우클릭 → **"DevNavi: 이 패키지 뭐야?"** |
| 🧭 | **이거 어디서 쓰여?** | VSCode Reference Provider로 함수·변수·클래스 참조를 수집 → 파일:라인 목록 + AI 요약을 인라인으로. 거대한 AI 생성 코드에서 흐름 잡을 때. | 심볼 드래그 or 커서 위치 → **`Ctrl+Alt+U`** · 우클릭 → **"DevNavi: 이거 어디서 쓰여?"** |

> **UX 통일**: "이 XX 뭐야?" 계열(코드 / 용어 / 에러 / 패키지 / 사용처) 전부 **드래그 → 단축키 or 우클릭 → 선택 영역 바로 아래 인라인 스레드**로 통일했습니다. 페이지 전환 없음.

> **용어 사전 저장**: AI 설명이 마음에 들면 인라인 스레드 하단의 **💾 사전에 저장** 버튼으로 커스텀 용어로 저장 → 다음부턴 로컬 히트 (토큰 0원).

> **코드 가이드 컨텍스트**: `이 코드 뭐야?` 호출 시 같은 파일의 **import/의존 선언만** 같이 보내서 연결 관계 감을 잡게 해요 (토큰은 조금만 늘음).
